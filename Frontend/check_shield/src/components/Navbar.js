...
import { Shield, Menu, X, Phone, Mail, Link as LinkIcon, Users, LayoutDashboard, TrendingUp } from "lucide-react"
...
const Navbar = () => {
  ...
  const navItems = [
    ...
    { path: "/reports", label: "Reports", icon: <TrendingUp className="h-4 w-4" />},
        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden md:flex space-x-1">
            {navItems.map((item) => {
              ...
              const isSpecial = item.path === "/reports" || item.path === "/community"
              ...
              return (
                <Link
                  ...
                  className={`flex items-center space-x-2 px-4 py-2 rounded-lg transition-colors ${
                    isSpecial
                      ? isActive
                        ? "text-slate-700 text-indigo-700 bg-indigo-50" // <-- stays even after unhover
                        : "bg-gradient-to-r from-indigo-500 to-purple-500 text-white shadow-md"
                      : isActive
                      ? "text-slate-700 text-white bg-gradient-to-r from-indigo-500 to-blue-500"
